...
def init_distributed_mode(args):
    ...
    # launched naively with `python pretrain.py`
...
class ImageAugmentation(object):
    def __init__(self, local_crops_number, dataset):
        match dataset:
            case "CIFAR10":
                self.normalize = k.Normalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010], keepdim=True)
            case "ImageNet":
                self.normalize = k.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225], keepdim=True)
            case _:
                self.normalize = k.Normalize(torch.zeros(3), torch.ones(3), keepdim=True)
    ...
    def __call__(self, x):
        ...
        img = x_tmp / 255.0
        crops = [self.normalize(img), self.transform_global1(img), self.transform_global2(img)]
        for _ in range(self.local_crops_number):
...
class SummaryWriterCustom(SummaryWriter):
    def __init__(self, log_dir: Union[str, Path] = ".", plot_size: int = 8):
        super().__init__(log_dir=log_dir)
        self.plot_size = plot_size

    def add_image_grid(self, tag, original, images, epoch, global_step):
        fig = create_image_grid(original, images, epoch, self.plot_size)
        self.add_figure(tag, fig, global_step)


def create_image_grid(original, images, epoch, size):
    to_pil = transforms.ToPILImage()
    denorm = kornia.augmentation.Denormalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010])
    width = len(images) + 1
    height = min(original.size(0), size)
    figure = plt.figure(figsize=(width, height))
    plt.subplots_adjust(hspace=0.5)

    titles = [f"orig@{epoch}"] + [f"global {n + 1}" for n in range(2)] + [f"global {n + 1}" for n in range(width)]
    total = 0
    for i in range(height):  # orig_img in enumerate(original_images, 1):
        all_images = [original[i].detach().cpu()] + [img[i].detach().cpu() for img in images]
        for j in range(len(all_images)):
            total += 1
            plt.subplot(height, len(all_images), total, title=titles[j])
            plt.xticks([])
            plt.yticks([])
            plt.grid(False)
            img = to_pil(denorm(all_images[j].unsqueeze(0)).squeeze())
            plt.imshow(img)
    figure.tight_layout()
    return figure
